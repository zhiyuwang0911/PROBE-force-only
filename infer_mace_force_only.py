#!/usr/bin/env python3
"""
Inference for force-only PROBE on MACE.

Requires:
  - MACE .model
  - best_force_only_model_*.pt from train_mace_force_only.py
  - test.xyz / .extxyz (reference forces needed for metrics)
...
def parse_args():
    p = argparse.ArgumentParser(description="Force-only PROBE inference (Fa + Fs)")
    p.add_argument("--mace-model", required=True)
    p.add_argument("--checkpoint", required=True, help="best_force_only_model_*.pt")
    p.add_argument("--test-xyz", required=True)
    p.add_argument("--output-dir", default="./probe_force_only_inference")
...
def run_inference(model, extractor, loader, device,
                  bins_fa, bins_fm, compute_metrics: bool):
    model.eval()
    ...
    for batch in tqdm(loader, desc="Inference"):
        ...
        for b in range(B):
            ...
            logits_fa, logits_fm = model(af, am)
            ...
            struct_rows.append({
                ...
                "mean_force_error_eV_A": float(struct_err.item())
                if torch.isfinite(struct_err) else float("nan"),
    ...
    metrics = {}
    if compute_metrics and store["force_mol"]["logits"]:
        for task in ("force_mol", "force_atom"):
...
def main():
    ...
    struct_rows, atom_rows, metrics = run_inference(
        model, extractor, loader, device, bins_fa, bins_fm,
        not args.no_metrics,
    )

    struct_fields = [
        "structure_index", "n_atoms", "mean_force_error_eV_A",
